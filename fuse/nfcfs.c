/*
  ...
  Adapted (c) 2015 Patrick Atamaniuk <patrick.atamaniuk@wibas.de>
...
#ifdef DEBUG
void hexdump (const void *addr, unsigned long len) {
    int i = 0;
    unsigned char buff[17];
    unsigned char *pc = (unsigned char*)addr;

    if (len == 0) { len = strlen(addr); }
...
#ifdef DEBUG
        printf("readdir org %s\n", d->entry->d_name);
        hexdump(d->entry->d_name, 0);
#endif
        int len = d->entry->d_namlen+1;
        newname = malloc(len);
        memset(newname, 0, len);
        if (normalize(d->entry->d_name, newname, len)) {
            free(newname);
            return -EIO;
        }
#ifdef DEBUG
        printf("readdir new %s\n", newname);
        hexdump(newname, 0);
#endif
